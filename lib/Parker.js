...
Parker.prototype.buffer = function (msg, length) {
    var ret = '';
...
Parker.prototype.report = function (data) {
    var data = this.run.call(this, data);

    var formattedData = _.map(data, function (val, key) {
        var key = humanize(key);

        return chalk.bold.cyan(key) + Parker.prototype.buffer(key) + '| ' + chalk.white(val);
    }).join('\n');
